	  ...
	  */
	  ...
	  websocket.on('connection', socket => {
	    ...
	    socket.on('dmx-load-preset', data => {
	      db.collection('dmx_presets').find({ instrument_id: data.instrument_id, preset_num: Number(data.preset_num), dmx_offset: Number(data.dmx_offset) }).then(dmx_preset => {
	        console.log("data " + JSON.stringify(dmx_preset));
	        socket.emit('dmx-load-preset-data', dmx_preset);
	      });
	
	      db.collection('dmx_presets', function (err, collection) {
	        collection.findOne({ instrument_id: data.instrument_id, preset_num: Number(data.preset_num), dmx_offset: Number(data.dmx_offset) }, { dmx_data: 1, _id: 0, instrument_id: 0, preset_num: 0, dmx_offset: 0 }, function (err, result) {
	          console.log("result " + JSON.stringify(result));
...
//# sourceMappingURL=0.e23c3323756df2349c79.hot-update.js.map