...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.post('/api/issues', (req, res) => {
	  ...
	  db.collection('issues').insertOne(_issue2.default.cleanupIssue(newIssue)).then(result => db.collection('issues').findOne({ _id: result.insertedId }).limit(1).next()).then(savedIssue => {
	    res.json(savedIssue);
	  }).catch(error => {
	    console.log(error);
	    res.status(500).json({ message: `Internal Server Error: ${error}` });
...
//# sourceMappingURL=0.3a46fbaa9931ae510d7a.hot-update.js.map