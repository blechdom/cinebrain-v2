...
/***/ (function(module, exports, __webpack_require__) {
	...
	/*app.post('/api/load_dmx_presets', (req, res) => {
	  console.log("what req? " + JSON.stringify(req.body) + " and what res? " + JSON.stringify(res.body));
	  db.collection('dmx_presets').find({instrument_id: req.body.instrument_id, preset_num: req.body.preset_num, dmx_offset: req.body.dmx_offset}).toArray()
	  .then(dmx_presets => {
	    console.log("res " + JSON.stringify(dmx_presets));
	    res.json({ dmx_data: dmx_presets.dmx_data });
	  })
	  .catch(error => {
	    console.log(error);
	    res.status(500).json({ message: `Internal Server Error: ${error}` });
	...
	*/
	app.get('/api/load_dmx_presets', (req, res) => {
	  console.log("what req? " + JSON.stringify(req.query.instrument_id) + " and what res? " + JSON.stringify(res.body)); // preset_num: Number(req.query.preset_num), dmx_offset: Number(req.query.dmx_offset)
	  db.collection('dmx_presets').find({ instrument: req.query.instrument_id }).toArray().then(dmx_presets => {
	    console.log("res " + JSON.stringify(dmx_presets));
	    res.json({ dmx_data: dmx_presets.dmx_data });
	  }).catch(error => {
	    console.log(error);
...
//# sourceMappingURL=0.e1db1d3db84990e5f8e0.hot-update.js.map