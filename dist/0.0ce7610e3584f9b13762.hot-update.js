...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.get('/api/dmx_presets', (req, res) => {
	  // const instrument = req.query.instrument_id;
	  //console.log("instrument is " + instrument_id);
	...
	class DMXWashGroup1 extends _react2.default.Component {
	  ...
	  static dataFetcher(_ref) {
	    ...
	    return fetch(`${urlBase || ''}/api/dmx_presets${parameters}`).then(response => {
	      if (!response.ok) return response.json().then(error => Promise.reject(error));
	      return response.json().then(data => this.state.dmx_data);
	    });
	  }
	
	  loadPreset(preset) {
	
	    DMXWashGroup1.dataFetcher({ preset_num: preset, instrument_id: this.state.instrument_id, dmx_offset: this.state.dmx_offset }).then(data => {
	      const parameters = data.ParametersMenu.records;
	      parameters.forEach(parameter => {
	        this.parameterOptions.push(_react2.default.createElement(
	          'option',
...
//# sourceMappingURL=0.0ce7610e3584f9b13762.hot-update.js.map