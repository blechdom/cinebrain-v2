	  ...
	  */
	  ...
	  websocket.on('connection', socket => {
	    ...
	    socket.on('dmx-load-preset', data => {
	      db.collection('dmx_presets').find({ instrument_id: data.instrument_id, preset_num: Number(data.preset_num), dmx_offset: Number(data.dmx_offset) }).toArray().then(dmx_preset => {
	        console.log("data " + JSON.stringify(dmx_preset));
	        let dmx_data = dmx_preset;
	        socket.emit('dmx-load-preset-data', dmx_preset.dmx_data);
	      });
	    });
...
//# sourceMappingURL=0.7eab1228eb5f7b1a5d9c.hot-update.js.map