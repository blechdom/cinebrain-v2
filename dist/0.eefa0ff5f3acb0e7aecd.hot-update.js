	  ...
	  */
	  ...
	  websocket.on('connection', socket => {
	    ...
	    socket.on('dmx-save-preset', data => {
	      db.collection('dmx_presets').update({ instrument: data.instrument_id, preset_num: data.preset_num, dmx_offset: data.dmx_offset }, data.dmx_data, { upsert: true });
	    });
	    socket.on('dmx-load-preset', data => {
	      db.collection('dmx_presets').find({ instrument: data.instrument_id, preset_num: Number(data.preset_num), dmx_offset: Number(data.dmx_offset) }).toArray().then(dmx_preset => {
	        console.log("data " + JSON.stringify(dmx_preset));
	        let dmx_data = dmx_preset;
...
//# sourceMappingURL=0.eefa0ff5f3acb0e7aecd.hot-update.js.map